...
public class InterfaceController {
	...
	public Map<String, Object> pay(HttpServletRequest request,
			...
			@RequestParam("userPassword") String userPassword,
			@RequestParam("userCard") String userCard) throws Exception {
	...
	public Map<String, Object> test(HttpServletRequest request,
			HttpServletResponse response)
			throws FailingHttpStatusCodeException, MalformedURLException,
			IOException, InterruptedException, NotFoundException {
		logger.warn("bigyoung开始淘宝识别二维码");
		new CrawlerUtil();
		// 生成UUid 用于区分浏览器
		...
		File path = new File(request.getSession().getServletContext()
				.getRealPath("/upload")
				+ "/");
		TextPage page = webClient
				.getPage("https://qrlogin.taobao.com/qrcodelogin/generateQRCode4Login.do");
		JSONObject jsonObject = JSONObject.fromObject(page.getContent());
		UnexpectedPage paerwm = webClient.getPage("https:"
				+ jsonObject.get("url"));
	...
	public Map<String, Object> tabLogin(HttpServletRequest request,
			HttpServletResponse response,
	...
	public List<String> getAddress(WebClient webClient) throws Exception {
		...
		WebRequest requests = new WebRequest(
				new URL(
						"https://buyertrade.taobao.com/trade/itemlist/asyncBought.htm?action=itemlist/BoughtQueryAction&event_submit_do_query=1&_input_charse"));
		List<NameValuePair> lists = new ArrayList<NameValuePair>();
		...
		for (int i = 0; i < array.size(); i++) {
			try {
				...
				JSONArray operations = obj.getJSONObject("statusInfo")
		...
		// 分页
		String totalPage = JSONObject.fromObject(str).getJSONObject("page")
				.getString("totalPage");
	 ...
	 * @throws IOException
	 */
	public List<String> getDetail(int count, WebClient webClient)
	...
	public String getAddress(HtmlPage html) {
		...
		try {
			...
			JSONArray array = JSONObject.fromObject(detailData)
					.getJSONObject("basic").getJSONArray("lists");
			for (int i = 0; i < array.size(); i++) {
				JSONObject json = array.getJSONObject(i);
				if ("收货地址".equals(json.getString("key"))) {
					text = json.getJSONArray("content").getJSONObject(0)
							.getString("text");
					return text;
				}
				;
	...
	// public Map<String,Object> tabLogin1(HttpServletRequest request,
	// HttpServletResponse response,@RequestParam("UserName") String
	// UserName,@RequestParam("UserPwd") String UserPwd)throws
	// FailingHttpStatusCodeException, MalformedURLException,IOException,
	...
	// //如果返回结果继续执行
	// WebRequest webRequest=new WebRequest(new
	// java.net.URL(crawlerUtil.XuexinPOST));
	// List<NameValuePair> list=new ArrayList<NameValuePair>();
