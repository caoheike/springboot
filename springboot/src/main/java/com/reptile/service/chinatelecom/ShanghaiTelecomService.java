...
public class ShanghaiTelecomService {
	...
	public Map<String, Object> afterLogin(HttpServletRequest request) {
		...
		try {
			HtmlPage nextPage = webClient.getPage(
					"http://www.189.cn/dqmh/ssoLink.do?method=skip&platNo=93507&toStUrl=http://service.sh.189.cn/service/query/bill");
	...
	public Map<String, Object> getDetial(String phoneNumber, String servePwd, HttpServletRequest request, String code,
		...
		try {
			HtmlPage detailPage = webClient.getPage(
					"http://service.sh.189.cn/service/service/authority/query/billdetail/validate.do?input_code=" + code
							+ "&selDevid=" + phoneNumber + "&flag=nocw&checkCode=验证码");
			...
			if (a.contains(str)) {
				...
				for (int i = 0; i < num; i++) {
					...
					WebRequest webRequest = new WebRequest(
							new URL("http://service.sh.189.cn/service/service/authority/query/billdetailQuery.do?begin=0&end=10&flag=1&devNo="
									+ phoneNumber + "&dateType=his&bill_type=SCP&moPingType=LOCAL&queryDate=" + month
									+ "&startDate=&endDate="));
					Thread.sleep(2000);
					...
					if (click.asText().contains("sumRow") && click.asText().contains("sumTime")) {
						...
						WebRequest webRequestResult = new WebRequest(
								new URL("http://service.sh.189.cn/service/service/authority/query/billdetailQuery.do?begin=0&end="
										+ pageEndNum + "&flag=1&devNo=" + phoneNumber
										+ "&dateType=his&bill_type=SCP&moPingType=LOCAL&queryDate=" + month
										+ "&startDate=&endDate="));
				...
				WebRequest webRequestnow = new WebRequest(
						new URL("http://service.sh.189.cn/service/service/authority/query/billdetailQuery.do?begin=0&end="
								+ pageEndNum + "&flag=1&devNo=" + phoneNumber
								+ "&dateType=now&bill_type=SCP&moPingType=LOCAL&queryDate=" + month + "&startDate="
				...
				if (succ.asText().contains(str2) && succ.asText().contains(str3)) {
					...
					WebRequest webRequestnowSucc = new WebRequest(
							new URL("http://service.sh.189.cn/service/service/authority/query/billdetailQuery.do?begin=0&end="
									+ pageEndNum + "&flag=1&devNo=" + phoneNumber
									+ "&dateType=now&bill_type=SCP&moPingType=LOCAL&queryDate=" + month + "&startDate="
									+ startDate + "&endDate=" + endDate));
