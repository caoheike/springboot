package com.reptile.service.chinatelecom.newpostanalysis;
		...
		try {
			...
			for (int i = 0; i < 7; i++) {
				...
				if (result1.toString().equals("2")) {
					logger.warn("-----------------重庆电信:" + userName + ":该用户在" + beforMonth(i) + "月，账单出现故障，故障原因："
				...
				post1.setRequestHeader("Referer", "http://cq.189.cn/new-bill/bill_xd?fastcode=02031273&cityCode=cq");
