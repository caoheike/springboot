...
public class ZhejiangTelecomService {
	...
	public Map<String, Object> zheJiangLogin(HttpServletRequest request, String phoneNumber) {
		...
		try {
			...
			HtmlPage choosePage = webClient.getPage(
					"http://www.189.cn/dqmh/ssoLink.do?method=linkTo&platNo=10012&toStUrl=http://zj.189.cn/zjpr/cdr/getCdrDetailInput.htm");
			choosePage
					.executeJavaScript(
							"$j('#cdrmonth').append(\"<option value= " + nowDate + ">" + nowDate + "</option>\")")
					.getNewPage();
			choosePage.executeJavaScript(
					"$j('#cdrmonth').append(\"<option value= " + GetMonth.beforMon(nowYear, nowMonth, 1) + ">"
							+ GetMonth.beforMon(nowYear, nowMonth, 1) + "</option>\")");
			choosePage.executeJavaScript(
					"$j('#cdrmonth').append(\"<option value= " + GetMonth.beforMon(nowYear, nowMonth, 2) + ">"
							+ GetMonth.beforMon(nowYear, nowMonth, 2) + "</option>\")");
			choosePage.executeJavaScript(
					"$j('#cdrmonth').append(\"<option value= " + GetMonth.beforMon(nowYear, nowMonth, 3) + ">"
							+ GetMonth.beforMon(nowYear, nowMonth, 3) + "</option>\")");
			choosePage.executeJavaScript(
					"$j('#cdrmonth').append(\"<option value= " + GetMonth.beforMon(nowYear, nowMonth, 4) + ">"
							+ GetMonth.beforMon(nowYear, nowMonth, 4) + "</option>\")");
			choosePage.executeJavaScript(
					"$j('#cdrmonth').append(\"<option value= " + GetMonth.beforMon(nowYear, nowMonth, 5) + ">"
							+ GetMonth.beforMon(nowYear, nowMonth, 5) + "</option>\")");
	...
	public Map<String, Object> zheJiangDetial(HttpServletRequest request, String phoneNumber, String servePwd,
			String name, String idCard, String code, String longitude, String latitude, String uuid) {
		Map<String, Object> map = new HashMap<String, Object>(16);
		// 验证码 判断 =========================================
		// 从session中获得webClient
		...
		try {
			...
			HtmlPage newPage = (HtmlPage) choosePage
					.executeJavaScript("generateCdrType(2,\"18\"," + phoneNumber + ",\"574\",\"移动电话\",\"4-3LL8EAT\")")
					.getNewPage();

			Thread.sleep(5000);

			HtmlInput randpsw = (HtmlInput) choosePage.getElementByName("cdrCondition.randpsw");
			// 3.
			...
			for (int i = 0; i < intz; i++) {
				...
				if (firstPage.asText().contains("我的清单详情")) {
					...
					for (int j = 1; j < pageNum + 1; j++) {
						...
						HtmlPage result = (HtmlPage) firstPage.executeJavaScript("javascript:sub(" + j + ")")
								.getNewPage();
						// 第j页
