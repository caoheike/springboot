...
public class ShanDongTelecomService {
		...
		if (attribute == null) {
			...
			try {
				...
				WebRequest requests = new WebRequest(new URL(
						"http://www.189.cn/dqmh/ssoLink.do?method=linkTo&platNo=10016&toStUrl=http://sd.189.cn/selfservice/account/returnAuth?columnId=0210"));
				...
				dataMap.put("CodePath", request.getScheme() + "://" + request.getServerName() + ":"
						+ request.getServerPort() + "/SDDXimageCode/" + fileName);
	...
	public Map<String, Object> getDetailMes(HttpServletRequest request, String userIphone, String imageCode,
			String userName, String userCard, String phoneCode, String userPassword, String longitude, String latitude,
			String uuid) {
		...
		if (attribute == null || htmlpage == null) {
			...
			try {
				...
				logger.warn(
						userIphone + "：---------------------山东电信获取详单完成--------------------本次获取账单数目：" + listData.size());
