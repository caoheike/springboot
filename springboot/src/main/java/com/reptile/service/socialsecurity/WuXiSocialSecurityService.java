...
@Service
public class WuXiSocialSecurityService {
	private Logger logger= LoggerFactory.getLogger(WuXiSocialSecurityService.class);
	//验证码
	public Map<String, Object> doGetVerifyImg(HttpServletRequest request) {
		Map<String, Object> data = new HashMap<String, Object>();
		WebClient webClient = new WebClientFactory().getWebClient();
		try {
			HtmlPage loginPage=webClient.getPage("http://www.hrsszj.gov.cn/PublicServicePlatform/index.action");
			Thread.sleep(1000);
			//读取页面验证码图片到本地
			HtmlImage img= (HtmlImage) loginPage.getElementById("logon_user");
			
			data.put("ImgPath", ImgUtil.saveImg(img, "right", "/verifyImages", "gif", request));
			
			request.getSession().setAttribute("ZhenJiangWebClient",webClient); 
			request.getSession().setAttribute("ZhenJiangLoginPage",loginPage); 
	...
	public Map<String, Object> doLogin(HttpServletRequest request, String idCard, String passWord, String catpy, String cityCode) {
		Map<String, Object> data = new HashMap<String, Object>();
		...
		try {
		  ...
		  //  data = new Resttemplate().SendMessage(data, "http://192.168.3.16:8089/HSDC/person/socialSecurity");
			...
			System.out.println("zhengjiangservice==="+e);
		}
		System.out.println("jieguo==="+data);
		return data;
	}
	/**
	 * 获取详情
	...
	public List<Map<String,Object>> getDetail(WebClient webClient,String type, HtmlPage detailpage) throws Exception{
		...
		System.out.println("allInfo==="+allInfo.size());
		return allInfo;
	}
    ...
    public List<List<String>>  getDetail(String beginTime,String endTime,WebClient webClient,String type, HtmlPage detailpage) throws Exception{
   	 ...
   	 if(info.size()>1) {
   		 //System.out.println("info=1=="+info.size());
    ...
    private double getCal(List<Map<String, Object>> unemploymentInsurance) {
